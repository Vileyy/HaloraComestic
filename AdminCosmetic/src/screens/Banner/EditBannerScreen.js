  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await updateBannerInFirebase(banner.id, {
        ...
        linkUrl,
        imageUrl: imageUri,
        imageChanged, // Thêm flag để biết liệu ảnh có được thay đổi hay không
      });
      Alert.alert("Thành công", "Banner đã được cập nhật thành công.", [
        { text: "OK", onPress: () => navigation.goBack() }
      ]);
    } catch (error) {
      Alert.alert("Lỗi", "Không thể cập nhật banner. Vui lòng thử lại sau.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Chỉnh Sửa Banner</Text>
        <View style={styles.headerRight} />
      </View>
      
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.formContainer}>
          {/* Phần xem trước và chọn ảnh */}
